...
func _entrar_braking() -> void:
	...
	distancia_total_frenado = abs(progress_objetivo - transition_llegada.progress)
...
func _estado_accelerating(delta: float) -> void:
	...
	if abs(progress_actual - transition_salida.progress) < epsilon:
...
func _estado_cruising(delta: float) -> void:
	...
	if abs(progress_actual - transition_llegada.progress) < epsilon:
